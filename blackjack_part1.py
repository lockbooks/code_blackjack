...
def deal(deck_func):
    ...
    for i in range(2):
        # перешиваем все карты в общей колоде
...
# очишаем экран
def clear():
    # проверка на тип ОС — Windows
    if os.name == 'nt':
        os.system('CLS')
    # проверка на тип ОС — Linux ли Unix-подобная
    if os.name == 'posix':
        os.system('clear')
...
def game():
    ...
    print("-" * 30 + "\n")
    # выводим слово ПОБЕДЫ жирным зелёнм шрифтом, слово ПОРАЖЕНИЯ - жирным красным,
    ...
    while True:
        ...
        if choice == 'д':
            ...
            # увеличиваем счётчик поражений, предлагем сыграть заново
            if total(player_hand) > 21:
                print('У вас перебор')
                losses += 1
                play_again()
        ...
        # при выборе остановиться ход переходит раздающему
            ...
            while total(dealer_hand) < 17:
                ...
                # увеличиваем счётчик побед, предлагем сыграть заново
                if total(dealer_hand) > 21:
                    print('У раздающего перебор, вы выиграли!')
                    wins += 1
                    play_again()
                    # добавляем break, чтобы цикл завершился после игры
                    break
...
# запускаем освновную функцию игры
game()
